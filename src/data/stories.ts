...
export interface Story {
  ...
  emoji: string;
  /** YYYY-MM-DD the story was added; newest stories are listed first */
  added?: string;
  chapters: Chapter[];
}
...
/*
 * Drop a new story file into src/data/library/ and it is picked up automatically.
 * Whitespace in library chapters is collapsed, so you can write paragraphs on separate lines.
 */
const libraryModules = import.meta.glob<{ default: Story }>('./library/*.ts', { eager: true });
const LIBRARY: Story[] = Object.keys(libraryModules)
  .sort()
  .map((key) => {
    const story = libraryModules[key].default;
    return { ...story, chapters: story.chapters.map((c) => ({ ...c, text: c.text.replace(/\s+/g, ' ').trim() })) };
  });

/** Every story, newest first (stories without an `added` date keep their original order at the end). */
export const STORIES: Story[] = [...LIBRARY, ...BUILT_IN]
  .map((story, index) => ({ story, index }))
  .sort((a, b) => (b.story.added ?? '').localeCompare(a.story.added ?? '') || a.index - b.index)
  .map((x) => x.story);
